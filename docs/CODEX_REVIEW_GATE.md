...
2. 再开一个后续测试 PR，确认 workflow 会在 `opened` / `synchronize` 时创建当前 head marker comment，并在 Codex clean top-level comment 带回本次 `codex-review-gate-token` 后写入 `codex/review-gate` status。
