...
- workflow 主要接受 marker comment 之后 `chatgpt-codex-connector` 发出的 clean top-level PR comment，例如 “didn't find any major issues”。
- 如果 Codex 只在 PR body 上留下新的 `+1` reaction，且这个 reaction 发生在 marker comment 之后，workflow 也可以把它当作 fallback clean signal。
- PR body 上旧的 `+1` reaction 不能作为当前 head 的通过信号，因为它没有 commit 绑定，也可能不会随下一轮 review 更新时间。
...
2. 再开一个后续测试 PR，确认 workflow 会在 `opened` / `synchronize` 时创建当前 head marker comment，并在 Codex clean top-level comment 或 marker 后的新 PR body `+1` 出现后写入 `codex/review-gate` status。
