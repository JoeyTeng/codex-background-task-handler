...
- Ready materialization is the production source of arm-pending markers: `bridge-preflight` may publish at most one eligible ready entry per wake, and that entry carries an issued bridge-scoped `arm_pending_requested` marker. If a heartbeat bridge is replaced before consuming the marker, the replacement bridge may reissue its own marker for the same prepared attempt; scanner/CAS fences still allow only one pending transition. `arm_accepted` markers are not pre-issued with ready entries; they can only be issued after the scanner has durably advanced the attempt to `arm_pending`. The accepted phase is also bridge-recoverable: a consumed pending marker from an older bridge proves the attempt reached `arm_pending`, while the replacement bridge receives its own `arm_accepted` marker after current binding and installation tokens are revalidated.
...
- scanner 只接受已签发 marker 对应的单一 canonical envelope hash，并在 fresh CAS 前复核当前 helper validation fingerprint 仍等于 marker 签发时的 fingerprint。prompt/user text、assistant final text、unissued marker、expired marker、helper fingerprint drift、hash 冲突的 duplicate trusted envelope、malformed trusted envelope、wrong field、wrong path、failed-CAS replay fence、truncate 或 inode drift 都不得推进 state；同 hash 的重复 trusted carrier evidence 只能幂等去重为一次 CAS。
...
- scanner 对新消费的 envelope 必须在同一个 immediate SQLite transaction 中重新确认 marker 仍为 issued、未过期、tokens / hash 匹配、当前 helper validation fingerprint 未漂移，且 scanner binding 的 path / identity / cursor 仍与本轮读取一致，然后才执行 `note-arm-pending` / `note-arm` CAS 并把 marker 标为 consumed；如果 marker 已被并发 reject / expire，helper fingerprint 已漂移，或 binding 已被并发 rebind / degrade / cursor advance，不得尝试 CAS。
