...
- scanner 只接受已签发 marker 对应的单一 canonical envelope hash。prompt/user text、assistant final text、unissued marker、expired marker、hash 冲突的 duplicate trusted envelope、malformed trusted envelope、wrong field、wrong path、failed-CAS replay fence、truncate 或 inode drift 都不得推进 state；同 hash 的重复 trusted carrier evidence 只能幂等去重为一次 CAS。
