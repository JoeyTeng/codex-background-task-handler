...
- scanner 发布 cursor、degrade binding、reject marker 时必须带 expected prior path / identity / byte cursor / line cursor / `updated_at` 与 active state 条件；如果并发 scan 已推进 cursor、degrade binding 或 operator 已 rebind，旧 scan 不得回退 cursor、清掉 degraded state、degrade replacement binding 或 reject marker。
