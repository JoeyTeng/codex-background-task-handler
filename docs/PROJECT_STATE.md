...
- Desktop bridge foundation 已开始落地，设计与实现边界见 [DESKTOP_BRIDGE_FOUNDATION.md](DESKTOP_BRIDGE_FOUNDATION.md)：
  ...
  - Desktop live preflight validation harness 已记录在 [DESKTOP_LIVE_PREFLIGHT_VALIDATION.md](DESKTOP_LIVE_PREFLIGHT_VALIDATION.md)，但完整 preflight/read validation 尚未通过；当前 `ready_threads` / `arm_pending_bindings` / `pause_due_bindings` entries 仍为空，caller heartbeat wake、automation mutation、ready attempt materialization、`note-arm*` / `note-boundary-crossed` 仍未实现
- Desktop live preflight validation 第一次真实 heartbeat 实测已记录在 [DESKTOP_LIVE_PREFLIGHT_EVIDENCE.md](DESKTOP_LIVE_PREFLIGHT_EVIDENCE.md)：
  - Heartbeat 能启动并执行本地 validation script / `cbth --version`，未观察到用户审批卡点
  - Mandatory `bridge-preflight` 失败在 `chmod 0700 /Users/hoteng/.cbth: Operation not permitted`；本地目录已经是 `drwx------`，说明当前 helper 对已私有目录的重复 chmod 会被 Desktop heartbeat 环境拦下
  - `read_transport_capability` 未写为 `validated`，installation state 仍为 `unknown`
