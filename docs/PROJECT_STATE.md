...
- 本仓用于在不修改上游 `codex` 的前提下验证和实现 `cbth` background task handler；当前主线已经完成 CLI dogfood v1 的主要能力、`v0.1.5` release，以及 Desktop bridge foundation。
...
- Next Steps:
  - Continue daemon upgrade safety from [daemon upgrade safety](project_journal/2026/05/2026-05-11-daemon-upgrade-safety-5721.md): PR4 app-server handoff, PR5 jobs drain, then the held `0.2.0` release PR.
