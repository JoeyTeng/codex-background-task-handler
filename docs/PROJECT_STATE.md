...
- Desktop heartbeat 已通过 no-DB helper path 验证 direct-file-read inbox 消费能力；`read_transport_capability=validated`，`artifact_read_capability` / `writeback_capability` 仍为 `unknown`。
...
- Blockers:
  ...
  - Desktop writeback helpers, boundary crossing, binding lifecycle, artifact reads, and several daemon deadline/recovery contracts remain incomplete.
- Evidence:
  - Release: `v0.1.0`
