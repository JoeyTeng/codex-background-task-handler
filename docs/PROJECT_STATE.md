...
- 已新增一个外围 `codex/review-gate` commit status：
  ...
  - clean pass 只接受 marker 后的 Codex clean top-level comment，且必须原样带回本次 marker 的 `codex-review-gate-token`
  - 通过前会重新确认 PR head 没变；PR body reaction 不作为通过信号，因为它不能绑定到当前 head
