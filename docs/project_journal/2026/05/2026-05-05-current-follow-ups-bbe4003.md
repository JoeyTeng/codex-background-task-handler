...
- Desktop heartbeat preflight attempts are recorded in [Desktop live preflight evidence](../../../DESKTOP_LIVE_PREFLIGHT_EVIDENCE.md): the first run failed at redundant `chmod 0700 /Users/hoteng/.cbth`; the no-op chmod repair got past that blocker, but the retry failed opening `/Users/hoteng/.cbth/run/startup.lock`.
- Decide and implement the next Desktop preflight permission boundary: either avoid daemon autostart / startup-lock writes when an already-running same-user daemon is available, or add a narrower helper path that can publish/read inbox snapshots without opening `~/.cbth/run/startup.lock`.
