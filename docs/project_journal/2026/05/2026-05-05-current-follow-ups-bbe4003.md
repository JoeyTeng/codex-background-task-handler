...
- Validate the transcript / tool-output relay side channel: interactive Desktop tool-output and heartbeat automation output both prove exact prefixed stdout can be recovered from rollout `function_call_output`. Only trusted carrier evidence can later feed the durable writeback CAS path; production consumption still needs durable scan cursors, replay protection, high-entropy nonce / lease / generation validation, and CAS mutation.
