...
- Relay marker consumption rechecks the current helper validation fingerprint immediately before fresh `note-arm-pending` / `note-arm` CAS, so markers issued by an older validated helper cannot keep advancing Desktop writeback after helper fingerprint drift has invalidated the installation capability proof.
