...
- `cbth cli run` 已启动 wrapper-owned sidecar client：
  ...
  - 自动 `turn/start` 前会重新执行 `thread/resume`，以 startup snapshot 为上限计算 effective permissions，并在 request params 中携带对应的 pinned `approvalPolicy` / `sandboxPolicy`；pinned sandbox 会保留或收紧协议要求的 `access` / `readOnlyAccess`、写根与 network access，包括 explicit no-write 把 `workspaceWrite` 降级为 `readOnly` 时仍使用两端 `readOnlyAccess` 的交集；权限 drift 会同时比较派生布尔值与 raw `approvalPolicy` / `sandbox` 细节，并输出 warning / 写 audit record
