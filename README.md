...
Managed CLI startup and `cbth doctor cli` also perform a soft Codex CLI compatibility check. `cbth` is currently validated against `codex-cli 0.129.x`; newer or unparsable versions produce a warning with diagnostic details, but execution continues and still relies on fail-closed protocol parsing for required fields.
