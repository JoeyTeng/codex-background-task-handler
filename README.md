...
- CLI protocol/TUI continuation foundations validated: the shared `app-server` route supports multi-client continuation on the same thread, and foreground TUI visibility is proven while the foreground remains on the same caller thread used in the PTY validation. The current first-version contract uses explicit fixed-thread binding for daemon-owned managed sessions, with durable `managed_session_id` / `bound_thread_id` bookkeeping and continuity-loss fail-closed behavior still remaining design contracts rather than end-to-end validated implementation. The route depends on capability probing around the experimental RPC surface, uses a loopback-only daemon-owned control plane, and keeps `turn/steer` as a gated optimization instead of a default path. Stronger local auth remains deferred while upstream loopback auth support is unavailable, and automatic thread rebind/switch routing is intentionally out of scope for v1.
