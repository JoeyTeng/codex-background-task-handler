...
The Rust CLI currently provides read-only store inspection commands, daemon-routed mutating job/batch/maintenance commands, hidden adapter-internal attempt and CLI-session commands, and a daemon control surface backed by a same-user Unix socket. The hidden `cli session bind` / `cli session note-activity` / `cli session inspect` commands are adapter-internal scaffolding for the future `cbth cli run` path: `bind` attach-or-creates a durable fixed-thread managed session with explicit risk-profile flags, re-attach advances `session_epoch` and clears prior activity proof, `note-activity` records the next sequential current-state proof such as `idle`, and `inspect` reads that record back. The hidden `attempt begin-cli-accept` command requires the adapter to provide a stable `--rpc-request-id` and a previously bound idle managed session; idempotent begin/accept paths still require that the stored attempt references a current valid managed session. The hidden `attempt observe-cli-turn` command is the store-facing terminal-event surface for a future app-server adapter: it records matching turn events, closes the batch only for an on-time `turn_completed`, and fail-closes late or failed/interrupted/replaced observations to `manual_resolution_only`. Daemon compatibility also requires the `attempt-dispatch`, `cli-session-dispatch`, and `cli-turn-observation-dispatch` capabilities so upgraded CLIs do not hand new attempt/session commands to an older daemon.
