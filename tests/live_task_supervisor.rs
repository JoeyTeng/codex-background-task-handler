...
fn live_codex_task_supervisor_e2e_is_opt_in() {
    if std::env::var("CBTH_RUN_LIVE_TASK_SUPERVISOR_E2E").as_deref() != Ok("1") {
        eprintln!("set CBTH_RUN_LIVE_TASK_SUPERVISOR_E2E=1 to run the live task supervisor e2e");
        return;
    }

    let codex_bin = std::env::var_os("CBTH_LIVE_CODEX_BIN").unwrap_or_else(|| "codex".into());
    let timeout = live_timeout("CBTH_LIVE_TASK_SUPERVISOR_E2E_TIMEOUT_SECONDS", 360);
    let home = temp_home();
    ...
    let (task_id, job_id) = submit_live_supervised_task(&home, &thread_id);
    wait_for_task_status(&home, &task_id, "succeeded", timeout);
    assert_task_logs_and_result_artifact(&home, &task_id, &job_id);
    wait_for_job_batch_close_reason(&home, &job_id, "delivered", timeout);
...
impl CbthRunGuard {
    fn spawn_new_thread(
        ...
        let mut child = Command::new(env!("CARGO_BIN_EXE_cbth"))
            ...
            .spawn()
            .expect("spawn cbth cli run live task supervisor");
...
fn submit_live_supervised_task(home: &TempDir, thread_id: &str) -> (String, String) {
    let task = cbth_json(
        home.path(),
        &[
            "task",
            ...
            thread_id,
            "--summary",
            "Live task-supervisor e2e: reply with exactly `CBTH_LIVE_TASK_SUPERVISOR_DELIVERED` and nothing else. Do not run tools.",
            "--delivery-read-only",
            "true",
            ...
            "/bin/sh",
            "-c",
            "printf 'CBTH_LIVE_TASK_SUPERVISOR_TASK_OUTPUT\n'",
        ],
    );
    let task = &task["task"];
    (
        task["task_id"].as_str().expect("task id").to_owned(),
        task["job_id"].as_str().expect("job id").to_owned(),
    )
}
...
fn wait_for_task_status(home: &TempDir, task_id: &str, status: &str, timeout: Duration) {
    let deadline = Instant::now() + timeout;
    loop {
        let inspected = cbth_direct_json(home.path(), &["task", "inspect", "--task-id", task_id]);
...
fn assert_task_logs_and_result_artifact(home: &TempDir, task_id: &str, job_id: &str) {
    let inspected = cbth_direct_json(home.path(), &["task", "inspect", "--task-id", task_id]);
    let stdout_log_path = inspected["task"]["stdout_log_path"]
        .as_str()
        .expect("stdout log path");
    let stdout_log = fs::read_to_string(home.path().join(stdout_log_path)).expect("stdout log");
    assert!(stdout_log.contains("CBTH_LIVE_TASK_SUPERVISOR_TASK_OUTPUT"));

    let job = cbth_direct_json(home.path(), &["job", "inspect", "--job-id", job_id]);
    let artifact_id = job["job"]["result_artifact_id"]
        .as_str()
        .expect("result artifact id");
    ...
    .expect("result artifact payload");
    assert!(payload.contains("command:"));
    assert!(payload.contains("CBTH_LIVE_TASK_SUPERVISOR_TASK_OUTPUT"));
}

fn wait_for_job_batch_close_reason(
    home: &TempDir,
    job_id: &str,
    close_reason: &str,
    timeout: Duration,
